...
const unProtectedRoutes = ["/community", "/communitypost/[communityPost_details]"];
...
const Community = ({ router, getCrud}) => {
  ...
  const unProtectedRoutes = ["/community", "/community/[add_query]"];
        ...
        <Container className="community-container mt-3 mb-5">
            <div style={styles.searchContainer}>
              <h2 className="" style={styles.title}>
                ...
                <span>{communityPostData?.title}</span>
              </h2>
            </div>
            <div data-grid="col-8" className="threadlist-filter-options">
              <div id="filterByTypeControls "  >
                <fieldset className="c-radio f-inline">
                  ...
                  <span style={{ fontSize: "18px" }}>Total Answers : {communityPostData?.__meta__?.total_post_replies}</span>
                  <span style={{ fontSize: "18px" }} className="ml-5">Total Views : {communityPostData?.views_counter}</span>
            ...
            {communitypostreply.map((item, index) => (
            <div className="card mt-4" style={{ background: "gainsboro", minHeight: "190px" }}>
                    ...
                    <div className="col-md-12 mb-2">
                      <div data-grid="col-12" className="thread-title single-line-text" aria-live="polite">
                        <span data-bi-id="thread-link" data-bi-index="0" data-bi-linkthreadid="63400f39-b2b7-4b28-88e9-569c183e0d63" href="https://answers.microsoft.com/en-us/windows/forum/all/windows-10-updates-fixes-2023/63400f39-b2b7-4b28-88e9-569c183e0d63"
                          style={{ color: "black" }}>Answer : {item?.__meta__?.total_helpful}  </span>
                      </div>
                    </div>
                    <div className="col-md-12 mt-3">
                      <span className="ml-5 " style={{ color: "black", fontSize: "20px", marginTop: "50", }}>Upvotes : {item?.__meta__?.total_helpful} </span>
                    </div>
                   <div className="row">
                      <div className="col-md-10" style={{ marginTop: 20, }}>
                        <span className="mt-5" style={{ color: "black", fontSize: "20px", }}>{item.description} </span>
                      </div>
              ...
              <div>
             {/* <a href="/community/add_query" ><button id="applyButton" name="button" className="btn btn-primary btn-sm mt-5" type="submit" fdprocessedid="vwz0lf">Add New Query</button> */}
                {/* </a> */}
              </div>
